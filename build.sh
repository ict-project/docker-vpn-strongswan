...
echo
SECRETS="$SECRETS --secret source=$SECRET_PREFIX_PK,target=/etc/ipsec.d/private/server-key.pem,mode=0400"
SECRETS="$SECRETS --secret source=$SECRET_PREFIX_PC,target=/etc/ipsec.d/certs/server-cert.pem,mode=0400"
SECRETS="$SECRETS --secret source=$SECRET_PREFIX_DN,target=/etc/ipsec.d/certs/server-dn.txt,mode=0400"
SECRETS="$SECRETS --secret source=$SECRET_PREFIX_SE,target=/etc/ipsec.secrets,mode=0400"
fi
